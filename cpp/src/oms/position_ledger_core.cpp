...
bool PositionLedgerCore::on_order_update(const internal::OrderStateUpdate& update,
    ...
    if (fill == nullptr) {
        ...
        error_message = "Fill status update is missing OrderFill payload";
        return false;
    }

    return apply_fill(update, *fill, error_message);
}

std::optional<PositionSnapshot>
PositionLedgerCore::market_position(internal::ExchangeId exchange,
...
bool PositionLedgerCore::apply_fill(const internal::OrderStateUpdate& update,
    ...
    if (market_ticker.empty()) {
        ++rejected_fill_count_;
        error_message = "Fill update is missing market_ticker";
        return false;
    }
    if (fill.fill_qty_lots <= 0) {
        ++rejected_fill_count_;
        error_message = "Fill update has non-positive fill_qty_lots";
        return false;
    }
    ...
    if (signed_fill_qty == 0) {
        ++rejected_fill_count_;
        error_message = "Fill update has unknown side";
        return false;
    }
...
internal::QtyLots PositionLedgerCore::abs_qty(internal::QtyLots value) {
    return value >= 0 ? value : -value;
}

internal::QtyLots PositionLedgerCore::side_to_signed_qty(internal::Side side,
